...
type LlmTraceProvider = 'anthropic' | 'openai' | 'google' | 'openrouter';
/** Which `LLMAgent` method initiated the call. */
type LlmTraceKind = 'callText' | 'callTools' | 'callWithTools' | 'callStructured';
/**
 ...
 */
declare function setLlmTraceSink(sink: LlmTraceSink | null): void;
/** Read the active sink. Mostly for tests. */
declare function getLlmTraceSink(): LlmTraceSink | null;
/**
 ...
 */
declare function emitLlmTraceEvent(event: LlmTraceEvent): void;
/** Strip `LLMToolDef[]` to the trace-friendly subset. */
declare function summarizeTools(tools: ReadonlyArray<LLMToolDef>): ReadonlyArray<{
    readonly name: string;
    readonly description: string;
}>;
/**
 ...
 */
declare function newLlmTraceId(): string;

export { type LlmTraceEvent, type LlmTraceKind, type LlmTraceProvider, type LlmTraceSink, emitLlmTraceEvent, getLlmTraceSink, newLlmTraceId, setLlmTraceSink, summarizeTools };
