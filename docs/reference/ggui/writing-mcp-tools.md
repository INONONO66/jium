...
> Local reference for Jium domain tool services. ggui is "invisible
> infrastructure": you don't write UI — you expose **tools** and ggui renders
> the interface. Tools arrive as **MCP servers** under `services/*`.

## Add a domain MCP (the worked path)

`services/api-gateway` is the reference implementation — a standalone
`@modelcontextprotocol/sdk` server over streamable HTTP, no `@ggui-ai/*` deps.
To make your own:

1. **Create it:** add `services/<your-domain>` and follow the API gateway service pattern. Rename the
   package in its `package.json` (`@jium/<domain>`) and pick a free port.
2. **Implement tools** in `src/handlers.ts`. Each tool:
   ...
   - Hold state however you like (the API gateway service uses an in-memory store; a
     real app uses a DB).
3. **Register it with the agent** (see AGENTS.md "Tools = MCP servers"):
   - **All SDKs:** just set `GGUI_<NAME>_MCP_URL` in `.env.example` / `.env.local`
     ...
     it as `<name>` — no `apps/agent/src/index.ts` edit needed (`ggui` is the
   ...
   - **Claude SDK only:** also add the tool prefixes to `DEFAULT_ALLOWED_TOOLS`
     in `apps/agent/src/agent.ts` (e.g. `mcp__<name>__menu_search`). The
     OpenAI + Google agents have no allowlist — the env var is enough.
4. **Run it:** nothing to wire — `pnpm dev` starts every `services/*`
   automatically To run just yours: `pnpm --filter ./services/<your-domain> dev`.
...
`docs/reference/ggui/ggui-tools.md` for the render tools and `docs/reference/ggui/ggui-json.md`
